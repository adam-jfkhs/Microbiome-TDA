...
We extracted homology groups $H_0$ (connected components), $H_1$
(one-cycles, i.e., loops), and $H_2$ (two-dimensional voids).
While the primary hypothesis---that dysbiosis disrupts cross-feeding
\emph{cycles}---maps onto $H_1$, we also computed $H_2$ as an
empirical stress test to determine whether higher-dimensional
topological features carry additional signal.
For the 80-node networks analysed here, the additional cost of $H_2$
was modest ($\sim$0.13\,s per diagram vs.\ $\sim$0.06\,s for $H_1$ only).
...
Six scalars were extracted from each $\mathrm{PD}_1$ (and, analogously,
six from $\mathrm{PD}_2$ for the $H_2$ analysis):
